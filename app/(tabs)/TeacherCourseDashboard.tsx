...
export default function TeacherCourseDashboard() {
  ...
  const renderCourseRow = ({ item: course }: { item: CourseWithStats }) => (
    <Animated.View entering={FadeInDown.duration(300)} style={styles.courseRow}>
      <View style={styles.courseInfoCell}>
      ...
      <View style={styles.courseActionsCell}>
        <TouchableOpacity style={styles.actionButtonSmall} onPress={() => navigateToEditCourse(course)}>
          <Edit size={18} color={colors.primary} />
        </TouchableOpacity>
        <TouchableOpacity style={styles.actionButtonSmall} onPress={() => navigateToManageLessons(course.id, course.title)}>
          <FileText size={18} color={colors.info} />
        </TouchableOpacity>
        <TouchableOpacity style={styles.actionButtonSmall} onPress={() => navigateToManageExam(course.id, course.title, course.exam_exists)}>
          <CheckSquare size={18} color={course.exam_exists ? colors.success : colors.warning} />
        </TouchableOpacity>
        <TouchableOpacity style={styles.actionButtonSmall} onPress={() => handleDeleteCourse(course)}>
...
const getStyles = (colors: typeof import('@/constants/Colors').default.light) =>
  StyleSheet.create({
    ...
    actionButtonSmall: {
      padding: 6, // Padding lebih kecil untuk tombol ikon
      marginLeft: 8, // Jarak antar tombol
    },
    emptyContainer: {
      flex: 1,
      justifyContent: 'center',
